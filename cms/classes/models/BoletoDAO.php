...
Class BoletoDAO
{
	...
	public function contador($condicao, $parametros)
	{
		 ...
		 * @var string $txtSql
		 */
		$txtSql		= "SELECT count(id) as quantidade FROM boleto n ".$condicao."";
		try
		{
     ...
     * @see Noticia
	 */
	public function inserir(Boleto $objNO)
	{
		
		$txtSql		= "INSERT INTO boleto
								   (boleto,
								    valor,
									dtPagamento
									)
		...
		try
		{
			...
			$sql->bindValue(':dtPagamento' 	, $objNO->getDtPagamento(), PDO::PARAM_STR);
			$sql->execute();
		}
		catch(PDOException $ex)
		{
	...
	public function ultimoId()
	{
		...
		$txtSql		= "SELECT max(id) as max FROM boleto";
		try
		{
	...
	public function excluir($codigo)
	{
		...
		$txtSql	= "DELETE FROM boleto WHERE id = :id";
		try
		{
	...
	public function buscar($codigo)
	{
		...
		if($resultado)
		{
			$objNO	= new Boleto( 	$resultado['id'],						  	
									$resultado['boleto'],
									$resultado['valor'],
									$resultado['dtPagamento'],
									$resultado['dtCadas']
							    );
			return $objNO;
		}
	 ...
	 * @see Noticia
	 */
	public function editar(Boleto $objNO)
	{	
	
		$txtSql		= "UPDATE boleto SET 
									boleto			= :boleto,
									valor	 		= :valor,
