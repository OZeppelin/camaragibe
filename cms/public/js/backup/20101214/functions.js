...
function mudaStatusTopico(id)
{
	$.post('topico/processo.php?t=status', 
	{
		id:id
	}, function(data){
		if (data[0].id == 0)
		{
			$("#topico"+id).attr("class",data[0].mensagem);
		}
	}, "json");	
}

function alteraCorrecao(int,des)
{
	$.post('desafio/processo.php?t=validar', 
	{
		desafio:des,
		internauta:int
	}, function(data){
		if (data[0].id == 0)
		{
			$("#resp"+int).attr("class",data[0].mensagem);
		}
	}, "json");	
}
